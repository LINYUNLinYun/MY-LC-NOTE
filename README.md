...
| 代码随想录-动态规划 | 18 | 45 | 53 | 只做hot100相关 |
| 代码随想录-单调栈 | 5 | 0 | 5 | 题目比较少全做了 |
| 代码随想录-图论 | 0 | 30 | 30 | 只刷有代表性的 |
| 总计 | 124 | 91 | 215 |  | 

**动态规划hot100:** 爬楼梯，最小花费爬楼梯，不同路径，不同路径 II，分割等和子集，
目标和，完全平方数，零钱兑换，单词拆分，打家劫舍，
买卖股票的最佳时机	，买卖股票的最佳时机II，最大子序和，编辑距离，最长上升子序列，
最长公共子序列，回文子串，最长回文子序列

**图论-卡码网：**
7 最大岛屿面积、10 高山流水、11 建造最大人工岛、14 有向图的完全连通、15 并查集理论基础、16 寻找存在的路径、17 多余的边、21 **拓扑排序(这道题很经典可以多留意)；**
有余力再刷的是：8 岛屿的总面积、9 沉没孤岛、12 海岸线计算、18 多余的边Ⅱ、22 Dijkstra朴素版、23 Dijkstra堆优化版、29 A*算法。
## 数据结构实现
...
知名困难题；
- dp做法：搞明白了还挺简单的 时间复杂度O(N)，空间复杂度O(N)。要维护每个下标的左右最高高度；
- 单调栈做法：单调栈的几个题都差不太多，可以参考代码随想录的单调栈部分，时间复杂度O(N)，空间复杂度O(N)；
- 双指针做法：最优做法，时间复杂度O(N)，空间复杂度O(1)。
...
## 72_edit_distance
编辑距离，数据科学的经典算法；
用动态规划会是个很妙的，具体看官方题解思路。简单地说，是说字符之间的变化**变成三种原子操作**，就成了状态机的转化——可以被动态规划给完成。
...
# 139_word_break
刚开始我想用贪心做，但是很遗憾，这题贪心绝对错；所以，最后学习了标准官答
...
最长上升子序列，经典题。二次方复杂度的做法我们想出来了，但是还有时间复杂度O(NlogN)的解法，很难，所以直接抄官方的即可。
...
## 494_target_sum
是416的变形(但要注意几个变换)：
1. 416是判断能不能分成两部分和相等，494是判断能不能分成特定和，一个只需判断能不能，一个还有找出方法有多少种
2. 一个是正整数，一个是非负整数所以循环和边界处理不同；

# 503_next_greater_element_ii
496题的变种，比较大的不同之处在于数组变成了一个环——所以遍历的时候要注意对下标取模、然后循环的结束条件也要注意下，我用一个单独的数组记录某变量是否被处理过？感觉有可能是多余的，也许可以再优化。
...
看题解的单调栈做法，很牛，竟然能做到On复杂度。普通做法是二次复杂度。
