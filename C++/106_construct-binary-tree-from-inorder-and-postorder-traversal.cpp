...
public:
    ...
    TreeNode* recursiveTree(vector<int>& inorder, vector<int>& postorder, TreeNode* root_node){
        if(inorder.size() == 0 && postorder.size()){
            return nullptr;
        }
        // 寻找当前根节点
        ...
        recursiveTree(inorder_left, postorder_left, root_node->left);
        recursiveTree(inorder_right, postorder_right, root_node->right);
    ...
    TreeNode* buildTree(vector<int>& inorder, vector<int>& postorder) {
        ...
        TreeNode* root;
        recursiveTree(inorder, postorder, root);
